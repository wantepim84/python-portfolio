...
def Story1(win):
    ...
    NewScreen = Toplevel(win, bg='Orange')
    ...
    SubmitButton = Button(NewScreen, text="Submit", background="Yellow", font=('Times', 12),
...
def Story2(win):
    def final(tl: Toplevel, profession, noun, feeling, emotion, verb):
        text = f'''
            One day, I aspired to become a {profession} and make a difference.
            My journey started with a simple {noun}, but the {feeling} of
            achieving my goals kept me going. Despite facing challenges, I
            embraced every {emotion} and continued to {verb} towards success.
        '''
...
Label(Screen, text=' Mad Libs Generator').place(x=150, y=20)

# creating buttons
Story1Button = Button(Screen, text='A memorable day', font=(
    "Times New Roman", 13), command=lambda: Story1(Screen), bg='Yellow')
Story1Button.place(x=140, y=90)
Story2Button = Button(Screen, text='Ambitions', font=(
    "Times New Roman", 13), command=lambda: Story2(Screen), bg='Yellow')
